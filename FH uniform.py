...
def lagrange_inversion_log_uniform(m, L=100, iter=100):
    ...
    a1 = uniform.moment(2, loc=-L, scale=m+L) / 2
    series = mean / a1
    A_hat = []
    for n in range(2, iter+1):
        series_old = series
        a_n1 = (-1)**(n + 1) * uniform.moment(n + 1, loc=-L, scale=m+L) / (n + 1) * math.factorial(n)
        a_n_hat = a_n1 / (n * a1)
        ...
        if abs(1 / series - 1 / series_old) <= 3e-3:
            break
    return 1 / series

M = np.arange(111, 131, 1)
